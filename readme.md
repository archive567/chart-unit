...
The data api is pretty much the same as linear now. I think there's
enough room in that library for the extra dimensionality aspect of
charting (what ggplot calls aesthetics, such as size, color and shape).

The bar charts have all become rectangle charts, with 4 dimensions
representing (x,y) (z,w) corners of the rectangle.

Given any remotness to others using this library, it's also a testing
ground for the tower project, hence the polymorph spagetti constraints.
You'll thanks me when you find rotating through a euclidean infinite
dimension space a one liner.

The charts themselves, with an attached `V2 Range a` and `[[R2 a]]` data
are themselves very close to numbers, and scaling operations could
already be directly applied to the chart.
...
-   configuration
-   a render of double-containered R2 data (has an x and y plane) as a
    concrete pretty picture
...
And produces a `Chart b` available to turn into an svg or for further
combinings.
...
The data out is a V4 with xz as the bucket range, and yw as 0 and the
bucket count. For a rectangle chart this translates to (x,y) as the
bottom left point and (z,w) as the upper right point of the rectangle.
...
This scaling ensures that axes absolute configuration has the same look
and feel. Hiding the scaling in a more general function:
...
The concrete manifestation of data on a page (the chart), and the
heads-up-display (the hud) are then trivially separated:
