...
type Chart b = (Renderable (Path V2 Double) b) => QDiagram b V2 Double Any
...
ured = (rybColor 11) `withOpacity` 0.5
...
scale :: (N t ~ Double, Transformable t, R2 (V t), Diagrams.Additive (V t)) =>
    Double -> t -> t
scale s = Diagrams.scale (if s==zero then eps else s)
