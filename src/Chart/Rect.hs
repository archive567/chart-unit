...
instance Default RectOptions where
    def = RectOptions 0.01 ugrey ublue
...
rectChart_ ::
    ...
    [f (Rect Double)] ->
    Chart b
rectChart_ optss asp rs =
    rectChart optss asp (fold $ fold <$> rs) rs
...
data Pixel = Pixel { pixelRect :: Rect Double, pixelColor :: AlphaColour Double }
...
pixelChart (Aspect asp) r pss =
  ...
  where
    projectPixels r0 r1 ps =
        zipWith Pixel
        (projectRect r0 r1 . pixelRect <$> ps)
        (pixelColor <$> ps)
...
pixelChart_ ::
    ...
    Chart b
pixelChart_ asp ps =
    pixelChart asp (fold $ fold . map pixelRect <$> ps) ps
...
pixelateChart ::
    ...
    (Pair Double -> Double) ->
    Chart b
pixelateChart opts asp xy f =
    pixelChart asp xy [pixelate opts xy f]
