...
hud (HudOptions p axes titles legends mr asp@(Aspect ar@(Ranges ax ay)) can) =
  ...
  where
    addTitle x (topts, t) =
        beside (placeOutside (titlePlace topts)) x
          (title_ asp topts t)
    addLegend x lopts = 
        ...
        then mempty
                ...
                (p_ $ pos' (legendAlign lopts) (legendPlace lopts) x') x') $
              legend lopts)
    ...
    canvas = rect_ can ar
    addAxis x aopts = case axisOrientation aopts of
...
withHud :: ( Foldable f ) =>
    ...
    [f (Pair Double)] ->
    Chart b
withHud opts renderer d = case hudRange opts of
  Nothing ->
      renderer (hudAspect opts) (foldMap space d) d <>
      hud (opts {hudRange = Just (foldMap space d)})
  Just r ->
      combine (hudAspect opts)
      [ UChart renderer r d
      , UChart
        (\asp _ _ ->
           hud
           ( opts {hudAspect=asp, hudRange=Just r}))
        r
        []
      ]
...
defXAxis = AxisOptions 1 Hori PlaceBottom
...
defYAxis = AxisOptions 1 Vert PlaceLeft
    (RectOptions 0 transparent (withOpacity black 0.1))
    0.02
...
axis :: ( ) =>
    ...
    Range Double ->
    Chart b
axis opts asp r = mo $ pad (axisPad opts) $ astrut $ centerXY $
  atPoints
    (pl <$> tickLocations)
    ((\x -> labelled (axisLabel opts) x (glyph_ (axisMark opts))) <$> tickLabels)
  `atop`
    arect (axisOrientation opts)
  where
    mo = moveOriginTo (p2 ((-lower asp)-width asp/2,0))
    arect Hori = rect_ (axisRect opts) (Ranges asp (Range 0 (axisRectHeight opts)))
    arect Vert = rect_ (axisRect opts) (Ranges (Range 0 (axisRectHeight opts)) asp)
    astrut x =
        beside (placeOutside (axisPlace opts)) x $
        case axisOrientation opts of
          Hori -> strutY (axisGap opts)
          Vert -> strutX (axisGap opts)
    pl = let gs = glyphSize (axisMark opts) in
      case axisPlace opts of
        PlaceBottom -> \x -> p2
            (x, (-0.5*gs) + (axisRectHeight opts) + (axisMarkStart opts))
        PlaceLeft -> \y -> p2
            ((-0.5*gs) + (axisRectHeight opts) + (axisMarkStart opts), y)
        PlaceTop -> \x -> p2
            (x, (0.5*gs) + (axisMarkStart opts))
        PlaceRight -> \y -> p2
            ((0.5*gs) + (axisMarkStart opts), y)
    (tickLocations, tickLabels) = case axisTickStyle opts of
      TickNone -> ([],[])
      {- To Do:
      ...
      TickRound n -> (project r asp <$> ticks0, precision 0 ticks0)
        where
          ticks0 = gridSensible OuterPos r n
...
title_ :: Aspect -> TitleOptions -> Text -> Chart b
title_ (Aspect (Ranges aspx aspy)) (TitleOptions textopts a p s) t =
    placeGap p
    (positioned (pos a p) (text_ (textopts {textAlignH = a}) t))
    s
  where
...
data LegendType b =
    ...
    LegendLine LineOptions Double |
    LegendGLine (GlyphOptions b) LineOptions Double |
    LegendRect RectOptions Double |
    LegendArrow (ArrowOptions Double) Double |
    LegendPixel RectOptions Double
...
legend opts =
    pad (legendOuterPad opts) $
    bound (legendRect opts) 1 $
    pad (legendInnerPad opts) $
    centerXY $
    vert (pad (legendRowPad opts))
  ...
  where
    ...
    legend__ (LegendLine c l, t) =
        hori identity [ oneline c (Pair (Pair 0 0) (Pair l 0))
                      , strutX (legendSep opts) 
                      , text_ (legendText opts) t]
    legend__ (LegendGLine gc lopts l, t) =
        hori identity [ (glyph_ gc `atop` oneline lopts (Pair (Pair (-l) 0) (Pair l 0)))
                      , strutX (legendSep opts) 
                      , text_ (legendText opts) t]
    ...
    legend__ (LegendArrow c s, t) =
        hori identity [ arrows c [Arrow zero (s *. one),
                                  Arrow (s *. one) zero]
                      , strutX (legendSep opts)
                      , text_ (legendText opts) t]
